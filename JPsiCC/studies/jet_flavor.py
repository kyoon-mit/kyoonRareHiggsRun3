...
def get_pandas_dataframes(sig_filename, bkg_filename, tree_name, branches, **kwargs):
...
def compute_correlation(sig_df, bkg_df, corr_var, save_path, matrix=True, scatter=True, terminal=False, **kwargs):
    ...
    if matrix:
        ...
        text_file_name = path.join(save_path, f'CORRELATION_MATRIX.txt')
        text_input = [f'<Signal>\n{sig_corr}',
                      '\n',
                      f'<Background>\n{bkg_corr}']
    ...
    if scatter:
        ...
        for col in list(set(sig_columns).intersection(bkg_columns)):
            ...
            plt.savefig(path.join(save_path, f'SCATTER_PLOT_sig_{corr_var}_VS_{col}.png'),
                        dpi=800, pad_inches=0.05)
            plt.close()
            ax_bkg = bkg_df.plot.scatter(x=col, y=corr_var, c='Blue', s=2.5, alpha=0.002)
            ax_bkg.set_title(f'Background: \n{corr_var} vs. {col}')
            plt.savefig(path.join(save_path, f'SCATTER_PLOT_bkg_{corr_var}_VS_{col}.png'),
                        dpi=800, pad_inches=0.05)
            plt.close()
            ...
            plt.savefig(path.join(save_path, f'SCATTER_PLOT_comb_{corr_var}_VS_{col}.png'),
                        dpi=800, pad_inches=0.05)
            plt.close()
    return
...
if __name__=='__main__':
    ...
    params = {
        ...
        'tree_name': 'Events',
        'branches': ['jetFar_CvL', 'jetFar_CvB'],
        'corr_var': 'jetFar_CvL'
    }
    ...
    ### Compute correlation
    compute_correlation(terminal=False, **params)