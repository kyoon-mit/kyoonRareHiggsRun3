...
for var in branches_plot:
    ...
    if var.endswith('pt') or var.endswith('mt'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 60, 0, 300), var)
    elif var.endswith('p') or var.endswith('energy'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 400, 0, 2000), var)
    elif var.endswith('eta'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 100, -10., 10.), var)
    elif var.endswith('phi'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 40, -4., 4.), var)
    elif var.startswith('dR'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 50, 0., 10.), var)
    elif var.startswith('deta'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 100, -10., 10.), var)
    elif var.startswith('dphi'):
        hist = rdf.Histo1D(ROOT.RDF.TH1DModel(var, var, 100, -10., 10.), var)
    else:
        hist = rdf.Histo1D(var)
    my_hists[var] = hist
