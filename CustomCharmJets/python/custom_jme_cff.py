...
def AddJetID(proc, jetName="", jetSrc="", jetTableName="", jetSequenceName=""):
  ...
  for modifier in run2_miniAOD_80XLegacy, run2_nanoAOD_94X2016: # type: ignore
  ...
  for modifier in run2_nanoAOD_94XMiniAODv1, run2_nanoAOD_94XMiniAODv2: # type: ignore
...
def ReclusterAK4CHSJets(proc, recoJA, runOnMC):
  ...
  cfg = {
    "jet" : "ak4pfchs",
    "inputCollection" : "slimmedJets", # <-- see PhysicsTools/PatAlgos/python/tools/jetCollectionTools.py; slimmedJets are AK4 PF CHS
    "genJetsCollection": "",
    "bTagDiscriminators": bTagDiscriminatorsForAK4,
    "minPtFastjet" : 0.,
  }
  ...
  #
  jetTableDoc = "Test trial."
  if runOnMC: # better turn this off!
    jetTableDoc += "runOnMC=True; For jets with pt < 8 GeV, only those matched to AK4 Gen jets are stored."
